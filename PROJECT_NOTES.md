...
- **Security headers:** `SecurityHeadersMiddleware` (registered last → outermost, so it also covers 401s and CORS preflights) adds `Content-Security-Policy`, `X-Frame-Options: DENY`, and `X-Content-Type-Options: nosniff` to every response. The CSP is same-origin with **no `unsafe-inline`**: `serve_frontend` generates a per-request base64 nonce, injects it into the one inline `<style>` and the one bare `<script>` tag of `index.html`, and stores it on `request.state.csp_nonce`; the middleware then emits `script-src 'self' 'nonce-…'` / `style-src 'self' 'nonce-…'` for the HTML page (all other responses get the same policy without the nonce terms). Consequence for frontend work: **no inline `onclick`/`onchange`/… attributes and no inline `style="…"` attributes** — static elements are bound via `addEventListener` in the IIFE's `bindStaticEvents()` (once per page load, not in `initApp()`), re-rendered table/candidate rows use `data-action`/`data-id` attributes with container-level event delegation, and one-off styles use the `.is-hidden`/`.text-*`/`.form-hint`/… utility classes at the end of the `<style>` block (JS may still set `el.style.*` via CSSOM — that is not affected by the CSP).
- **CORS:** `CORSMiddleware` is only registered when `PM_CORS_ORIGINS` (comma-separated origin list) is set; by default the API sends no CORS headers (the SPA is same-origin). The old `allow_origins=["*"]` + `allow_credentials=True` combination was removed on purpose (Starlette would echo any Origin back with credentials).
...
| `PM_CORS_ORIGINS` | *(empty = disabled)* | Comma-separated CORS origins (empty = no CORS headers; SPA is same-origin) |
...
- **Chart libraries are self-hosted:** `static/vendor/` holds chart.js 4.4.0, luxon 3.4.4, and chartjs-adapter-luxon 1.3.0 (byte-identical to the previously SRI-pinned jsdelivr artifacts — verified by matching sha384 SRI hashes). Same-origin, so the CSP needs no external script host and no `integrity` attributes. To bump a version, download the new minified file over `static/vendor/<file>` and update the `<script src>` tag in `index.html`.
