...
- **Security headers:** `SecurityHeadersMiddleware` (registered first → outermost, so it also covers 401s and CORS preflights) adds `Content-Security-Policy` and `X-Frame-Options: DENY` to every response. The CSP is same-origin by default with `script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net` (`unsafe-inline` is required — the SPA uses inline event handlers/style attributes throughout; the three CDN scripts are SRI-pinned with `integrity` + `crossorigin="anonymous"` in `index.html`), `connect-src 'self'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`.
...
- **CDN libraries are SRI-pinned:** the three jsdelivr `<script>` tags (chart.js, luxon, chartjs-adapter-luxon) carry `integrity="sha384-…"` + `crossorigin="anonymous"`, and the CSP's `script-src` allows only `https://cdn.jsdelivr.net` as an external source. If you bump a version, recompute the sha384 over the *served* bytes (`curl -s <url> | Get-FileHash -Algorithm SHA384`, then base64-encode the hex digest) — for auto-minified jsdelivr builds that's the CDN artifact itself, not the npm file.
